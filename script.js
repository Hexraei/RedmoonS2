document.addEventListener('DOMContentLoaded', () => {
    ...
    async function sendData(action, data, noCors = false) {
        try {
            ...
            // This specific error check is kept as a fallback, but the 'no-cors' mode change should prevent it.
    ...
    // Set 'noCors' to true for registration to improve submission reliability on all devices
    ...
    const getTshirtSelectionHTML = () => {
        ...
        return `<div class="container mx-auto px-4 sm:px-8 py-8 bg-white min-h-screen"><div class="text-center mb-4"><img src="./images/logo/redmoon_logo.png" alt="REDMOON" class="w-48 h-24 md:w-64 md:h-32 object-contain mx-auto" /><h2 class="text-2xl text-[#8B0909] mt-2 mb-4 font-bold small-caps tracking-wider">Official Survey</h2><p class="text-black text-sm md:text-base max-w-3xl mx-auto mb-4">To begin, please select your preferred designs from the curated collection below. Your choices will help us define the core style of the new MIT apparel line.</p><p class="text-black text-xs md:text-sm">There is no limit to the number of designs you can select.</p></div><div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6 mb-8">${cards}</div><div class="text-center"><button data-action="finalize-tshirts" ${state.selectedTShirts.length === 0 ? 'disabled' : ''} class="bg-black text-white px-8 py-2 text-sm hover:bg-gray-800 no-rounded font-bold w-full max-w-2xl small-caps disabled:opacity-50 disabled:cursor-not-allowed">Submit Selections</button></div></div>`;
    };
    ...
    root.addEventListener('click', async (e) => {
        ...
        if (action === 'submit-registration') {
            ...
            try {
                ...
                // This alert will now only show for actual network errors, not common submission issues.
